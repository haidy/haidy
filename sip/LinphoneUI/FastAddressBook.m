...
void sync_address_book (ABAddressBookRef addressBook, CFDictionaryRef info, void *context) {
    ...
    @synchronized (lAddressBookMap) {
        ...
        for (id lPerson in lContacts) {
            ABMutableMultiValueRef lPhoneNumbers = ABRecordCopyValue((__bridge ABRecordRef)lPerson, kABPersonPhoneProperty);
            for ( int i=0; i<ABMultiValueGetCount(lPhoneNumbers); i++) {
                ...
                Contact* lContact = [[Contact alloc] initWithRecord:(__bridge ABRecordRef)(lPerson) ofType:(__bridge NSString *)lLocalizedLabel];
...
-(FastAddressBook*) init {
    if ((self = [super init])) {
        ...
        ABAddressBookRegisterExternalChangeCallback (addressBook,sync_address_book,(__bridge void *)(mAddressBookMap));
        sync_address_book(addressBook,nil,(__bridge void *)(mAddressBookMap));
    }
    return self;
}
